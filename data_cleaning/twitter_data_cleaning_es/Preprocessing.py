...
def preprocess_file(origin_path, save_path, file_type, column="text", vocab=None):
    ...
    if file_type == "txt":
        ...
        if os.path.splitext(origin_path)[1] == "tsv":
            try:
                df = pd.read_csv(origin_path, header=None, index_col=False, sep="\t")
                ff = df.iloc[:,0].to_list()
            except: # Sometimes get this error for an unclear reason: ParserError: Error tokenizing data. C error: Buffer overflow caught - possible malformed input file.
                with open(origin_path, "r") as fin:
                    ff = fin.readlines()
            
        elif os.path.splitext(origin_path)[1] == "csv":
...
if __name__ == "__main__":
    path = "../../train_data/twitter_es/2019_03/tweets_{}.tsv" #14_to_31.tsv"

    origin_path = path.format("clean")
    save_path = path.format("processed_test")
