...
def load_word_vectors(file_destination):
    ...
    for line in f:
        ...
        if len(line)==1:
            continue
        ## RM: changed this line and removed the .lower() because I have cased vectors and I am dealing primarily with names
...
def simlex_analysis(word_vectors, language="english", source="simlex", add_prefixes=True):
    ...
    pair_list = []
    if source == "simlex":
        fread_simlex = codecs.open("attract_repel/evaluation/simlex-" + language + ".txt", 'r', 'utf-8')
    elif source == "simlex-old":
        fread_simlex = codecs.open("attract_repel/evaluation/simlex-english-old.txt", 'r', 'utf-8')
    elif source == "simverb":
        fread_simlex = codecs.open("attract_repel/evaluation/simverb.txt", 'r', 'utf-8')
    elif source == "wordsim":
        fread_simlex = codecs.open("attract_repel/evaluation/ws-353/wordsim353-" + language + ".txt", 'r',
...
def main():
    ...
    try:
        ...
        config_filepath = "./attract_repel/config/experiment_parameters.cfg"
